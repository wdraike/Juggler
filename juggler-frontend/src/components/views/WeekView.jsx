...
export default function WeekView({ selectedDate, dayPlacements, statuses, directions, onStatusChange, onExpand, gridZoom, darkMode, schedCfg, nowMins, onGridDrop, blockedTaskIds, onZoomChange }) {
      ...
      {days.map((d, i) => {
        ...
        return (
          <div key={d.key} style={{ flex: 1, borderRight: i < 6 ? `1px solid ${theme.border}` : 'none', minWidth: 0 }}>
            <div style={{ padding: '4px 4px', fontSize: 10, fontWeight: 600, color: theme.text, borderBottom: `1px solid ${theme.border}`, textAlign: 'center', background: d.isToday ? theme.accent + '15' : 'transparent' }}>
