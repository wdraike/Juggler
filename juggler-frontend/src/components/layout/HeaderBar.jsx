  ...
  return (
    ...
    <div ref={headerRef} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 'inherit', flexShrink: 0, ...(useOverflow ? { marginLeft: 'auto' } : {}) }}>
        ...
        {/* 999.1227: visible Undo affordance (desktop + mobile), driven by
            canUndo — dimmed/disabled when the undo stack is empty. */}
        {onUndo && <button onClick={onUndo} disabled={canUndo === false} style={{ ...btnStyle(theme, isMobile), ...(canUndo === false ? { opacity: 0.4, cursor: 'default' } : {}) }} title="Undo last action (Ctrl/Cmd+Z)" aria-label="Undo last action">&#x21A9;</button>}
