 ...
 *
 * Returns:
 *   weatherByDate[dateKey] = { high, low, precipPct, code, hourly[] }
 *   where hourly[i] = { hour, temp, precipProb, cloudcover, code }
 */
...
function parseWeather(data, unit) {
  var { time, temperature_2m, precipitation_probability, cloudcover, weathercode } = data.hourly;
  var byDate = {};
  ...
  for (var i = 0; i < time.length; i++) {
    ...
    if (!byDate[dateKey]) {
      byDate[dateKey] = { high: -Infinity, low: Infinity, precipPct: 0, code: 0, hourly: [] };
    }
    ...
    // Use most-severe weathercode for the day (higher codes = more severe)
    if (code > d.code) d.code = code;

    d.hourly.push({ hour, temp, precipProb: precip, cloudcover: cloudcover[i] || 0, code });
  }
