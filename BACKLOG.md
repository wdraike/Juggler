...
| ~~JUG-MED-11~~ | ~~Task card right-edge overflow — weather badge + count indicator clip at viewport boundary~~ | — | — | 2026-05-19 | **DONE** — `flexWrap: 'wrap'` + `minWidth: 0` on TaskCard Row 2 container + spacer (`flex: '1 1 0%'`); same fix on ListView date-group header. Badges wrap instead of clipping. 3 regression tests added (`TaskCard.overflow.test.jsx`); 99/99 pass. Human-verified at 375/768/1280px light+dark (commit `1665506`). |
| JUG-MED-12 | `/health` router: add mount-level auth guard (defense-in-depth) | — | — | 2026-05-20 | `app.js` mounts health router at `/health` and `/api/health` without outer `authenticateJWT`. Auth IS applied correctly inside each route handler (no live vulnerability). Risk: any new sub-route added to `health.routes.js` will be unauthenticated by default. Fix: add `router.use(authenticateJWT)` at the top of `health.routes.js`, or comment the mount sites documenting the per-route auth pattern. Found by peneloppy during weather cache fix review (2026-05-20). |
...
| JUG-MED-13 | TaskCard.jsx (List/Priority/Conflicts views) renders no 📌 pin badge for fixed tasks | juggler-frontend/src/components/tasks/TaskCard.jsx | — | 2026-05-23 | Pre-existing gap surfaced during sync-promotion fix review (Bird W-001). TaskCard.jsx has never rendered a pin badge — neither for `task.rigid`, `task.fixed`, nor `placement_mode === 'fixed'`. Fix: in Row 2 (line 116-207), add a 📌 span alongside recurring/reminder/flexed badges, gated by `task.fixed \|\| task.rigid \|\| task.placementMode === 'fixed' \|\| task.placement_mode === 'fixed'`. Mirror title from ScheduleCard.jsx:70. Affects List, Priority, and Conflicts views. |
| JUG-MED-14 | AllDayBanner.jsx renders no fixed indicator | juggler-frontend/src/components/views/AllDayBanner.jsx | — | 2026-05-23 | Pre-existing gap surfaced during sync-promotion fix review (Bird W-002). All-day cal-synced events (always `placement_mode='fixed'`) appear in banner across DayView, WeekView, ThreeDayView with no fixed indicator. |
| JUG-MED-15 | Migrate 6 downstream `task.when === 'allday'` sites to `placement_mode === 'all_day'` | juggler-backend/src/scheduler/runSchedule.js:2105; juggler-backend/src/controllers/cal-sync.controller.js:1422,1513,1550,1608; juggler-backend/src/lib/cal-adapters/gcal.adapter.js:214; juggler-backend/src/lib/cal-adapters/msft.adapter.js:298; juggler-backend/src/lib/apple-cal-api.js:231; juggler-frontend/src/components/views/CalendarView.jsx:268-269 | — | 2026-05-23 | Phase 9 enum redesign deprecated `when='allday'` but 6+ downstream consumers still read it. Sync adapters currently dual-write `when='allday'` + `placement_mode='all_day'` as a temporary bridge (see comments in gcal/msft/apple `applyEventToTaskFields`). Plan: migrate consumers to `placement_mode`, then drop the legacy `when` write. Centralize via shared `isAllDayTaskBackend()` predicate analogous to frontend `isAllDayTask.js`. |
| JUG-MED-16 | Investigate root cause of spurious adapter promotion-to-fixed (date-shift trigger) | juggler-backend/src/lib/cal-adapters/gcal.adapter.js:applyEventToTaskFields; juggler-backend/src/controllers/cal-sync.controller.js:_buildPullFields | — | 2026-05-23 | Sync-promotion fix shipped 2026-05-23 (commit 21ef012) prevents the SYMPTOM (anytime tasks getting silently locked + going overdue) by writing canonical `placement_mode=FIXED` instead of legacy `when='fixed'` + scheduler anchor gate on ANYTIME. Does NOT fix the upstream TRIGGER: adapter's `dateChanged || timeChanged` check fires when `event.startDateTime` differs from `task.date/time`. Evidence from sync_history shows specimen task t1779306283537zper shifted May 21 → May 22 between 15:13 and 23:02, then promotion fired. Unknown which of three candidate causes drove the shift: (a) scheduler rollover proposed new date + push wrote it + pull saw "external change", (b) push-pull race exposed mid-push state, (c) user actually moved event in provider UI. None verified. Investigation options: (1) instrument `applyEventToTaskFields` to log `event.lastModifiedBy` / equivalent provider metadata before promoting; (2) add cross-sync ordering guard — don't pull within N seconds of pushing the same task; (3) compare event hash against `last_pushed_hash` ledger BEFORE promoting (skip promotion when hash matches our last push — confirms WE were the source of the change). Recommended sequence: (3) first as it's a low-risk filter, then (1) for residual cases. |
