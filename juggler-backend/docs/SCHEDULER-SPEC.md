...
- **Code:** `unifiedScheduleV2.js:1172-1778` (per SCHEDULER-RULES.md §3.1 source cite). The "phases" are logical sections of one pass over a shared `dayOcc`:
  ...
  - **Phase 4 — Missed window:** TIME_WINDOW whose flex window is entirely past → dual-placed on grid with `_overdue=true` + also listed unplaced.
  - **Phase 5 — Past-anchored recurring:** recurring with `anchorDate < today` → force-placed at original date, `_overdue=true`.
  - **Phase 6 — Rigid forced:** still-unplaced fixed/rigid → force-placed at anchor with `_conflict=true, locked=true` (overlaps existing occupancy; last resort).
...
- **Status:** IMPLEMENTED
- **Tests:** `tests/unit/tpc-competition.test.js`, `tests/unit/tpc-spacing-guard.test.js`, `tests/scheduler/recurring-fixed-fallback.test.js`
