...
- `placement_mode = 'anytime'` → no time constraint. Placed wherever fits by priority/slack order. (The former `preferLatestSlot` day-end lane for past-anchor recurring tasks was REMOVED — 999.1559, David ruling 2026-07-12: exhausted window → unscheduled, never crammed at day end.)
...
- **`depReadyAbs` is computed once per scan, not per slot.** `findEarliestSlot` hoists a single `depReadyAbs = checkDeps ? computeDepReadyAbs(...) : -Infinity` at the top of the scan (`findLatestSlot` was REMOVED — 999.1559). Every candidate slot then does an O(1) comparison `absoluteMin(date, slot) < depReadyAbs → skip` rather than recomputing dependency state. This is the **scan-constancy invariant (A-001)**: within a single `findEarliestSlot` call, `placedById` and `statuses` are treated as read-only snapshots — placements are written by the *caller* after the scan returns, so the dependency floor cannot shift mid-scan and slot decisions remain internally consistent.
