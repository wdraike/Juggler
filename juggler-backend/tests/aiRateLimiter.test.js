...
describe('AI rate limiter — HTTP 429 on 3rd request in window', function() {
  ...
  beforeAll(function() {
    ...
    jest.doMock('../src/lib/task-write-queue', function() { return { isLocked: jest.fn().mockResolvedValue(false), discardQueuedWrites: jest.fn().mockResolvedValue(0), enqueueWrite: jest.fn().mockResolvedValue(), flushQueue: jest.fn().mockResolvedValue(), flushQueueInLock: jest.fn().mockResolvedValue(), splitFields: jest.fn(function(f) { return { schedulingFields: {}, nonSchedulingFields: f }; }), NON_SCHEDULING_FIELDS: [] }; });
