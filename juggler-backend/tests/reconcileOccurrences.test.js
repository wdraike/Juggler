...
describe('reconcileOccurrences.buildExistingGroups', () => {
  test('groups chunks of the same occurrence_ordinal together', () => {
    const rows = [
      row({ id: 'm-1',    master_id: 'm', occurrence_ordinal: 1, split_ordinal: 1, split_total: 2, date: '4/19' }),
      row({ id: 'm-1-2',  master_id: 'm', occurrence_ordinal: 1, split_ordinal: 2, split_total: 2, date: '4/19' }),
      row({ id: 'm-2',    master_id: 'm', occurrence_ordinal: 2, split_ordinal: 1, split_total: 1, date: '4/26' })
    ];
    ...
    expect(out['m'][1].date).toBe('4/19');
    expect(out['m'][2].chunkIds).toEqual(['m-2']);
  });

  test('excludes cal-linked rows from the group pool', () => {
    const rows = [
      row({ id: 'ok',    master_id: 'm', date: '4/19' }),
      row({ id: 'gcal1', master_id: 'm', date: '4/20', gcal_event_id: 'abc' }),
      row({ id: 'msft1', master_id: 'm', date: '4/21', msft_event_id: 'def' })
    ];
    const out = reconcile.buildExistingGroups(rows, parseDate);
  ...
  test('excludes non-pending rows (done/skip/cancel)', () => {
    const rows = [
      row({ id: 'ok',   master_id: 'm', date: '4/19', status: '' }),
      row({ id: 'done', master_id: 'm', date: '4/20', status: 'done' })
    ];
    const out = reconcile.buildExistingGroups(rows, parseDate);
    const groupIds = Object.values(out['m']).map(g => g.occId);
    expect(groupIds).toEqual(['ok']);
...
describe('reconcileOccurrences.matchOccurrences', () => {
  function makeGroup(opts) {
    return Object.assign({
      occOrd: 1, occId: 'x', date: '4/19', dateObj: parseDate('4/19'),
  ...
  test('exact-date match preserves id with no move', () => {
    const existing = { m: groupMap(makeGroup({ occOrd: 1, occId: 'm-1', date: '4/19', dateObj: parseDate('4/19'), chunkIds: ['m-1'] })) };
    const desired = [{ id: 'm-5', sourceId: 'm', date: '4/19' }];
  ...
  test('nearest-first match emits a move entry', () => {
    ...
    const existing = { m: groupMap(
      makeGroup({ occOrd: 20, occId: 'm-20', date: '4/26', dateObj: parseDate('4/26'), chunkIds: ['m-20'] }),
      makeGroup({ occOrd: 21, occId: 'm-21', date: '5/3',  dateObj: parseDate('5/3'),  chunkIds: ['m-21'] })
    )};
    const desired = [{ id: 'm-5', sourceId: 'm', date: '4/19' }];
    const out = reconcile.matchOccurrences(desired, existing, parseDate);
    expect(out.occIdOverrides).toEqual({ 'm-5': 'm-20' });
    expect(out.occurrenceMoves).toHaveLength(1);
    expect(out.occurrenceMoves[0].newDate).toBe('4/19');
  ...
  test('exact-date matches take priority over nearest-first', () => {
    ...
    const existing = { m: groupMap(
      makeGroup({ occOrd: 15, occId: 'm-15', date: '4/20', dateObj: parseDate('4/20'), chunkIds: ['m-15'] }),
      makeGroup({ occOrd: 20, occId: 'm-20', date: '4/26', dateObj: parseDate('4/26'), chunkIds: ['m-20'] })
    )};
    const desired = [
      { id: 'm-100', sourceId: 'm', date: '4/19' },
      { id: 'm-101', sourceId: 'm', date: '4/26' }
    ];
    const out = reconcile.matchOccurrences(desired, existing, parseDate);
  ...
  test('excess existing groups go unmatched (will be deleted by caller)', () => {
    ...
    const existing = { m: groupMap(
      makeGroup({ occOrd: 1, occId: 'm-1', date: '4/20', dateObj: parseDate('4/20'), chunkIds: ['m-1'] }),
      makeGroup({ occOrd: 2, occId: 'm-2', date: '4/22', dateObj: parseDate('4/22'), chunkIds: ['m-2'] }),
      makeGroup({ occOrd: 3, occId: 'm-3', date: '4/24', dateObj: parseDate('4/24'), chunkIds: ['m-3'] })
    )};
    const desired = [{ id: 'm-99', sourceId: 'm', date: '4/22' }];
    const out = reconcile.matchOccurrences(desired, existing, parseDate);
  ...
  test('excess targets stay unmatched (will be inserted by caller)', () => {
    ...
    const existing = { m: groupMap(
      makeGroup({ occOrd: 1, occId: 'm-1', date: '4/22', dateObj: parseDate('4/22'), chunkIds: ['m-1'] })
    )};
    const desired = [
      { id: 'm-10', sourceId: 'm', date: '4/20' },
      { id: 'm-11', sourceId: 'm', date: '4/22' },
      { id: 'm-12', sourceId: 'm', date: '4/24' }
    ];
    const out = reconcile.matchOccurrences(desired, existing, parseDate);
  ...
  test('idempotent: same inputs twice produce same result', () => {
    const buildExisting = () => ({ m: groupMap(
      makeGroup({ occOrd: 1, occId: 'm-1', date: '4/20', dateObj: parseDate('4/20'), chunkIds: ['m-1'] }),
      makeGroup({ occOrd: 2, occId: 'm-2', date: '4/27', dateObj: parseDate('4/27'), chunkIds: ['m-2'] })
    )});
    const desired1 = [
      { id: 'new-10', sourceId: 'm', date: '4/20' },
      { id: 'new-11', sourceId: 'm', date: '4/27' }
    ];
  ...
  test('only matches within the same master', () => {
    const existing = {
      m1: groupMap(makeGroup({ occOrd: 1, occId: 'm1-1', date: '4/19', dateObj: parseDate('4/19'), chunkIds: ['m1-1'] })),
      m2: groupMap(makeGroup({ occOrd: 1, occId: 'm2-1', date: '4/20', dateObj: parseDate('4/20'), chunkIds: ['m2-1'] }))
    };
    const desired = [
      { id: 'm1-new', sourceId: 'm1', date: '4/20' }, // closer to m2-1, but must match m1
      { id: 'm2-new', sourceId: 'm2', date: '4/19' }
    ];
    const out = reconcile.matchOccurrences(desired, existing, parseDate);
