  ...
  describe('TERMINAL_STATUSES (task_instances)', function() {
    test('TERMINAL_STATUSES is frozen and contains expected values', function() {
      ...
      expect(taskStatus.TERMINAL_STATUSES).toEqual(
        expect.arrayContaining(['done', 'cancel', 'cancelled', 'skip', 'pause', 'missed'])
      );
    });

    test('TERMINAL_STATUSES has exactly 6 entries (no accidental additions)', function() {
      expect(taskStatus.TERMINAL_STATUSES.length).toBe(6);
    });

    test('isTerminalStatus returns true for all terminal values', function() {
      ['done', 'cancel', 'cancelled', 'skip', 'pause', 'missed'].forEach(function(s) {
