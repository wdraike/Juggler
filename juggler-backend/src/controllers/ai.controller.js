...
exports.handleCommand = async (req, res) => {
  try {
    ...
    const taskLines = (tasks || []).map(t => {
      ...
      return t.id + '|' + (t.date || 'TBD') + '|' + (t.time || '') + '|' + st + '|' + (t.project || '') + '|' + t.text + '|' + (t.where || '') + '|' + (t.when || '') + '|' + (t.pri || '') + (t.recurring ? '|recurring' : '') + '|' + (t.dur || 30) + 'm' + (t.deadline ? '|deadline:' + t.deadline : '') + (t.earliestStart ? '|start:' + t.earliestStart : '') + (deps.length ? '|deps:' + deps.join(',') : '');
    }).join('\n');
