...
function applyEventToTaskFields(event, tz, currentTask) {
  ...
  // 999.4671: provider transparency (Google "Free") NEVER decides placement_mode.
  // A synced event is a time block; only a Juggler-side change makes it a reminder.
  // This drops BOTH the old transparent→REMINDER demotion (which silently turned
  // every all-day and Gmail-derived event into a dur=0 marker the scheduler would
