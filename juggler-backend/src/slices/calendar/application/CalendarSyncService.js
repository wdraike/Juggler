/**
 * CalendarSyncService — sync orchestration extracted from cal-sync.controller.js
 * (999.942).
 *
 * Encapsulates the full calendar sync lifecycle: fetch events from all connected
 * providers, reconcile against the local ledger, push/pull changes, and persist
 * results. The controller becomes a thin HTTP wrapper.
 *
 * REFACTOR mode — NO BEHAVIOR CHANGE. Every step is the same sequence the legacy
 * controller ran, moved out of the HTTP handler so the controller can become thin.
 */
...
var { taskHash, userHash, isoToJugglerDate, toMySQLDate, DEFAULT_TIMEZONE, callWithRateLimit } = require('../../lib/cal-sync-helpers');
...
var { rowToTask, safeParseJSON, fetchTasksWithEventIds } = require('../../controllers/task.controller');
var { getConnectedAdapters } = require('../facade');
var { createLogger } = require('@raike/lib-logger');
var logger = createLogger('cal-sync.service');
...
/**
 * Main sync orchestration — the full sync lifecycle.
 *
 * @param {Object} user - The authenticated user (must have .id and .timezone)
 * @param {Object} [query] - Query parameters (e.g. { trigger: 'auto' })
 * @returns {Promise<Object>} Sync stats object
 */
async function sync(user, query) {
  query = query || {};
  var userId = user.id;
  ...
  var triggerType = query.trigger === 'auto' ? 'auto' : 'manual';
...
/**
 * Lightweight check if any connected calendar has changes.
 * Uses Google sync tokens to avoid fetching all events.
 * Returns { hasChanges: true/false, providers: { gcal: { hasChanges }, ... } }
 */
async function hasChanges(user) {
  var connectedAdapters = getConnectedAdapters(user);
  if (connectedAdapters.length === 0) {
    return { hasChanges: false, providers: {} };
  }
  ...
  for (var i = 0; i < connectedAdapters.length; i++) {
    ...
    try {
      ...
      var token = await adapter.getValidAccessToken(user);
      var check = await adapter.hasChanges(token, user);
  ...
  var userId = user.id;
  var lastSynced = [
    user.gcal_last_synced_at,
    user.msft_cal_last_synced_at,
    user.apple_cal_last_synced_at,
...
/**
 * Retrieve sync history for the current user.
 * @param {Object} user - The authenticated user
 * @param {Object} [query] - Query params ({ limit, offset, syncRunId })
 * @returns {Promise<Object>} { runs: [...] }
 */
async function getSyncHistory(user, query) {
  query = query || {};
  var userId = user.id;
  var runLimit = Math.min(parseInt(query.runs) || 20, 50);
 ...
 * @param {Object} user - The authenticated user
 * @param {Object} [query] - Query params ({ days: N })
 * @returns {Promise<Object>} Audit report
 */
async function audit(user, query) {
  query = query || {};
  var userId = user.id;
  var userRow = await getDb()('users').where('id', userId).first();
  var _tz = userRow.timezone || DEFAULT_TIMEZONE;

  var days = Math.min(parseInt(query.days, 10) || 7, 60);
  var now = new Date();
