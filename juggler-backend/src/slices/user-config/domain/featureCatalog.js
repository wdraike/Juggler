/**
 * featureCatalog.js — StriveRS's configurable-features catalog (domain data).
 *
 * 999.1192 (JUG-HEX-SLICES-CALL-CONTROLLERS): moved VERBATIM from
 * controllers/feature-catalog.controller.js so the user-config facade stops
 * requiring an HTTP-layer controllers/* module for static domain data (that
 * require also closed the user-config facade ↔ feature-catalog.controller
 * cycle). The controller re-exports `CATALOG` unchanged for its external
 * consumers (payment-service fetch path).
 *
 * ONLY features with actual implementation are listed here.
 * value_order: higher = more prominent in plan cards (100+ headline,
 * 50-99 important, 1-49 secondary)
 */
