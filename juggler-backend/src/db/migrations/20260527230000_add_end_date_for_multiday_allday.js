/**
 * Add end_date column for multiday all-day tasks (JUG-MED-17).
 *
 * This migration adds end_date support to task_instances and task_masters tables,
 * allowing all-day tasks to span multiple days (e.g., a conference from Monday to Wednesday).
 *
 * Changes:
 *   1. Add end_date DATE column to task_instances
 *   2. Add end_date DATE column to task_masters
 *   3. Update tasks_v view to include end_date
 *   4. Update tasks_with_sync_v view to include end_date
 *
 * The end_date is nullable. When null, the task is a single-day all-day task.
 * When set, the task spans from date to endDate (inclusive).
 */

exports.up = async function(knex) {
  await knex.transaction(async (trx) => {
    // STEP 1 — Add end_date column to task_instances
    await trx.schema.table('task_instances', function(t) {
      t.date('end_date').nullable().comment('For multiday all-day tasks: the end date (inclusive). Null = single-day task.');
    });

    // STEP 2 — Add end_date column to task_masters
    await trx.schema.table('task_masters', function(t) {
      t.date('end_date').nullable().comment('For multiday all-day tasks: template-level end date. Null = single-day task.');
    });

    // STEP 3 — Drop and recreate tasks_v with end_date
    ...
    // STEP 4 — Recreate tasks_with_sync_v with end_date
    await trx.raw(`
...
exports.down = async function(knex) {
  await knex.transaction(async (trx) => {
    // Drop views first (they reference the columns we're dropping)
    await trx.raw('DROP VIEW IF EXISTS tasks_with_sync_v');
    await trx.raw('DROP VIEW IF EXISTS tasks_v');

    // Drop columns from task_instances and task_masters
    await trx.schema.table('task_instances', function(t) {
    ...
    // Recreate views without end_date (using the previous migration's view definition)
    // This is a simplified recreation - in practice, you'd want to use the exact previous view definition
    await trx.raw(`
      CREATE VIEW tasks_v AS
      SELECT
        m.id                         AS id,
    ...
    // Recreate tasks_with_sync_v without end_date
    await trx.raw(`
      CREATE VIEW tasks_with_sync_v AS
      SELECT
        v.id, v.user_id, v.task_type, v.text, v.dur, v.pri, v.project, v.section,
