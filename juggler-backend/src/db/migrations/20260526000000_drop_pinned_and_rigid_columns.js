/**
 * Drop legacy date_pinned, prev_when, and rigid columns.
 *
 * Prerequisites:
 *   1. Run AUDIT-date_pinned-mismatch.sql and confirm the SELECT returns 0.
 *   2. Confirm no application code references these columns (done in When-mode redesign).
 *
 * This migration also rebuilds tasks_v and tasks_with_sync_v without the
 * dropped columns so the views remain consistent.
 *
 * DO NOT RUN until the data audit SQL confirms zero mismatches.
 */
exports.up = async function(knex) {
  await knex.transaction(async (trx) => {
    // STEP 1 — Drop views in dependency order before altering tables.
    ...
    // STEP 4 — Recreate tasks_with_sync_v without date_pinned and prev_when.
    await trx.raw(`
...
exports.down = function() {
  // Intentionally no rollback — columns contained no surviving business logic
  throw new Error('This migration cannot be rolled back safely');
};
