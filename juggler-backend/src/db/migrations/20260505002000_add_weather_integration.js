/**
 * Weather integration — Phase 1 foundation:
 *   1. Create weather_cache table (hourly forecast cache keyed by lat/lon grid)
 *   2. Add weather condition columns to task_masters
 *   3. Recreate tasks_v + tasks_with_sync_v to expose the new columns
 */

exports.up = async function(knex) {
  // ── 1. weather_cache ───────────────────────────────────────────────────────
  await knex.schema.createTable('weather_cache', function(t) {
    t.bigIncrements('id');
    t.decimal('lat_grid', 5, 2).notNullable();
    t.decimal('lon_grid', 5, 2).notNullable();
    t.datetime('fetched_at').notNullable();
    t.datetime('expires_at').notNullable();
    t.specificType('forecast_json', 'MEDIUMTEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci').notNullable();
    t.index(['lat_grid', 'lon_grid', 'expires_at'], 'idx_weather_cache_coords_exp');
  });

  // ── 2. task_masters weather columns ────────────────────────────────────────
  await knex.raw(
  ...
  // ── 3. Recreate views ──────────────────────────────────────────────────────
  // Drop dependents first (tasks_with_sync_v depends on tasks_v)
...
exports.down = async function(knex) {
  ...
  await knex.raw(
    "ALTER TABLE task_masters " +
    "  DROP COLUMN weather_precip, " +
    "  DROP COLUMN weather_cloud, " +
    "  DROP COLUMN weather_temp_min, " +
    "  DROP COLUMN weather_temp_max, " +
    "  DROP COLUMN weather_temp_unit"
  );

  // Restore views from previous migration state (placement_mode_stored)
  await knex.raw(`
    CREATE VIEW tasks_v AS
    SELECT
      m.id                         AS id,
