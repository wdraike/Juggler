/**
 * Rebuild `tasks_v` with LEFT JOIN so detached instances (master_id=NULL,
 * produced by the ON DELETE SET NULL FK in migration 20260415010700) still
 * appear in the view. Same for `tasks_with_sync_v` which layers on top.
 *
 * Detached instances have NULL for all master-only fields (text, pri,
 * project, recur, etc.). That's a known limitation of the two-table split —
 * the legacy `tasks` flat row kept its own copy of template fields, so
 * history rows retained full context; the normalized schema doesn't.
 * Callers that care about detached-instance display can denormalize
 * on write (copy fields into a dedicated archival master) or rely on
 * the legacy `tasks` table until it's dropped.
 */
exports.up = async function(knex) {
  ...
  // Rebuild tasks_with_sync_v on top of the new tasks_v
  await knex.raw(`
...
exports.down = async function(knex) {
  // Restore the prior INNER JOIN version (20260415010400 + 20260415010500)
  await knex.raw('DROP VIEW IF EXISTS tasks_with_sync_v');
  await knex.raw('DROP VIEW IF EXISTS tasks_v');
  // Re-run prior view definitions would be tedious; this `down` is a no-op
  // placeholder. In practice, rolling back 20260415010700 deletes NULL rows
  // which lets the prior INNER JOIN view recreate without losing data.
  throw new Error('Down migration not supported; restore via prior migration files.');
};
