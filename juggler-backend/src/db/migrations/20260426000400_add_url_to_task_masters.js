/**
 * Add `url` column to task_masters for linking tasks to external resources
 * (email threads, docs, issue links, etc.). Nullable, VARCHAR(2000) to cover
 * long URLs with query strings.
 *
 * Views (tasks_v, tasks_with_sync_v) rebuilt to expose the new column on
 * both the template branch and the instance branch (instances inherit from
 * their master).
 */
exports.up = async function(knex) {
  await knex.schema.alterTable('task_masters', function(table) {
    table.string('url', 2000).nullable().collate('utf8mb4_unicode_ci');
  });

  await knex.raw('DROP VIEW IF EXISTS tasks_with_sync_v');
  await knex.raw('DROP VIEW IF EXISTS tasks_v');

  // tasks_v mirrors 20260426000300_add_placement_mode with `url` added.
  await knex.raw(`
...
exports.down = async function(knex) {
  ...
  console.log('[MIGRATION] down: re-run 20260426000300 to restore view definitions');
};
