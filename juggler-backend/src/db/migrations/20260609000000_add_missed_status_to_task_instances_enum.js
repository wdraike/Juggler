 ...
 *
 * NOTE (2026-06-09 repair): MySQL implicitly commits on every DDL statement, so
 * wrapping these ALTERs in knex.transaction() raises error #805 ("Transaction was
 * implicitly committed, do not mix transactions and DDL"). The statements are run
 * directly and made idempotent instead, so the migration also applies cleanly on a
 * DB seeded from a prod schema snapshot (which already carries the purge index).
 */
...
exports.up = async function(knex) {
  ...
  try {
    ...
    if (/doesn't exist|not found|unknown constraint|check constraint/i.test(error.message)) {
  ...
  if (!(await knex.schema.hasColumn('task_instances', 'completed_at'))) {
    await knex.raw('ALTER TABLE task_instances ADD COLUMN completed_at DATETIME NULL');
    console.log('Added completed_at column');
  } else {
    console.log('completed_at column already exists');
  }

  // Step 4: Add index for purge operations (idempotent — prod snapshots already have it)
  if (!(await indexExists(knex, 'task_instances', 'idx_task_instances_purge'))) {
    await knex.raw('ALTER TABLE task_instances ADD INDEX idx_task_instances_purge (user_id, status, completed_at) COMMENT "Sharded purge query support"');
    console.log('Added purge index');
  } else {
    console.log('Purge index already exists, skipping...');
  }
...
exports.down = async function(knex) {
  ...
  try {
    await knex.raw('ALTER TABLE task_instances DROP COLUMN completed_at');
    console.log('Dropped completed_at column');
  } catch (error) {
    if (/doesn't exist|not found|unknown column/i.test(error.message)) {
  ...
  if (await indexExists(knex, 'task_instances', 'idx_task_instances_purge')) {
    await knex.raw('ALTER TABLE task_instances DROP INDEX idx_task_instances_purge');
