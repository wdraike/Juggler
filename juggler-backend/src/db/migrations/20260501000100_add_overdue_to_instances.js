/**
 * Issue #44: add `overdue` column to task_instances.
 *
 * When the scheduler can't place a task that has a scheduled_at (i.e. it was
 * previously placed but no longer fits), the old behaviour was to set
 * unscheduled=1. The new behaviour:
 *   - Keep unscheduled=0 (task stays visible at its last scheduled position)
 *   - Set overdue=1 (frontend shows a red indicator in-place)
 *
 * Views are rebuilt to expose the new column in tasks_v / tasks_with_sync_v.
 */
exports.up = async function(knex) {
  await knex.schema.table('task_instances', function(table) {
    table.tinyint('overdue').notNullable().defaultTo(0);
  });

  // Rebuild views to include the new column. Drop both (with_sync depends on
  // tasks_v), then recreate in the correct order.
...
exports.down = async function(knex) {
  ...
  throw new Error(
    'Down-migration drops the overdue column and leaves views dropped. ' +
    'Re-run the latest view-creating migration to restore tasks_v / tasks_with_sync_v.'
  );
};
