...
async function runScheduleAndPersist(userId, _retries, options) {
  ...
  // 10. (W4 — DB single source) The separate `user_config` schedule_cache blob
  // write was removed. The scheduler still persists task placements to
  // task_instances above; the placed/unplaced view is now DERIVED from the
  // task read model (GET /api/tasks → deriveSchedulePlacements), so there is no
  // longer a second cache copy to keep in sync.
