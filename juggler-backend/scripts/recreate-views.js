// One-shot recovery: recreate tasks_v and tasks_with_sync_v after the
// 20260425000100 migration dropped them then failed before CREATE. Uses the
// schema from 20260425000000 (without the still-unreleased preferred_time
// boolean column) AND includes the source_id fix:
//   CASE WHEN m.recurring = 1 THEN m.id ELSE NULL END AS source_id
// which stops one-off tasks from carrying a bogus source_id pointing at
// themselves.
var db = require('../src/db');

(async function() {
  await db.raw('DROP VIEW IF EXISTS tasks_with_sync_v');
  await db.raw('DROP VIEW IF EXISTS tasks_v');
