...
## 2026-07-02T22:30:00Z — sched-audit-L5 — abby
- type: requirement
- artifact: juggler/docs/REQUIREMENTS.md R50.1 (amendment), R50.9, R50.10, R50.11; juggler-backend/docs/SCHEDULER-SPEC.md §C (Overdue/Forward-Roll subsystem)
- change: Registered David's 2026-07-02 D-A/D-B/D-C product rulings + the REG-26 code fix as requirements (docs-only, no code change this leg). (1) R50.1 gets a SECOND amendment (D-A): a never-placed one-off task with a real deadline now follows the SAME "pin to deadline, never roll forward, unscheduled+overdue" rule juggy4 already gave recurring/split — closes the R50.1-vs-code contradiction (REG-12/Q1) in favor of R50.1's literal text. (2) New R50.9 (D-B): Unscheduled-lane rows are resolvable in place (done/skip/cancel enabled directly, no "schedule first" gate) — frontend confirmed shipped; **status marked `partial`, not `implemented`** because this doc-verification pass found the backend `UpdateTaskStatus.js` TERMINAL_REQUIRES_SCHEDULE guard was never updated to match for plain one-offs / non-`rolling`-cadence recurring instances (still 400s) — referred to ernie/backlog, not fixed here. (3) New R50.10 (D-C): force-placed rigid/FIXED slots are now reserved against scheduler-driven intrusion (closes REG-24/a3-02), reminder-type items still coexist, rigid-vs-rigid explicit double-book is still allowed, overdue rigid stays pinned at its slot + in the overdue list. (4) New R50.11: registers the previously-unregistered OVD-1..4 forward-roll subsystem (REG-17 — fully shipped, zero prior R-number) and folds in the REG-26 fix (persistence-sweep `computeEffectiveDeadline` flipped from max() back to the LOCKED min(recurrence-period-boundary, window-close), closing the F9 doc↔code contradiction; Phase-3 `isMissedPreferredTime` scoped to `!isFlexibleTpc` so flexible-TPC items roam forward within-cycle instead of dead-ending). (5) Added an "OPEN QUESTIONS" subsection under R50 listing 4 still-unruled empty cells A1 flagged (all_day+overdue, chain-member+overdue, weather-blocked+deadline-passed, cal-sync-lock+overdue) as awaiting product ruling — deliberately not authored as requirements.
- rationale: closes the sched-audit leg's L5 (requirements register hygiene) work item; keeps REQUIREMENTS.md/SCHEDULER-SPEC.md as the accurate single source of truth for already-shipped, audit-verified behavior so future legs don't re-litigate settled rulings or re-file already-fixed bugs against stale docs.
- supersedes: 2026-06-26T13:00:00Z — juggler-sweep-overdue — oscar (that entry recorded computeEffectiveDeadline as MAX(period-boundary, window-close) "because min() makes the period-boundary extension dead" — this was itself later found to contradict the actual LOCKED SCHEDULER-SPEC.md decision (David, 2026-06-23, min()) and to be a live production bug (REG-26); the code has been fixed to min() and this notice reconciles the KG to match)
- challenge-id: none
- resolves: sched-audit AUDIT-REGISTER.md REG-12, REG-17, REG-26 (Q1/Q6 in "Questions for David" — both now answered/closed)
